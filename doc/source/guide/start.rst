...
`here <https://bitbucket.org/akunze3/pytrajectory/downloads>`_. 
...
After the iteration has finished we have acces to callable functions for the system variables
`T.x(t)` and input variables `T.u(t)` where `t` has to be in `(a,b)`.

In this example we get a solution that satisfies the default tolerance for the boundary values 
of :math:`10^{-02}` after the 7th iteration step with 320 spline parts. But PyTrajectory enables 
