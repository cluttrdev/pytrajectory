...
class Trajectory:
	...
	def iteration(self):
		...
		## Liste der Indizes der DGLn die durch Kollokation gelöst werden sollen
		## (untere Enden der Integr. Ketten)
		self.l2=l2

		#initiliase splines
		self.init_splines()	## splines symbolisch anlegen
		guess=np.empty(0)	#this will be the guess for the collocation equation system
		self.c_list=np.empty(0) ## ex liste der symbol. koeffs, jetzt ein array
		...
		while(max(abs(self.Diff))>self.epsilon_tol and ii<=self.v_max):
			...
			for cc in self.coeff:
				...
				if(self.splines[cc].type=='x'):
					...
					##!! rausfinden warum linalg.solve nicht geht
	...
	def find_integrators(self):
		...
		#[3.4]
		#search for equations in f like \dot x_i = x_[i+1]
		#just call ff with symbolic variables and checks for any equation which equals exact a symbolic variable
		#thata will be stored in a dict, which will be used in init_splines()
	...
	def equation_system(self):
    	...
    	## delta = 1 : nur knotenpunkte
    	## delta = 2 : auch die jeweilige Mitte noch mit
	...
	def solve(self):
		#solves the collocation equation system by using class Solver [4]
